...
private:
	// one cell has one value and two pointers: one for
	// next cell and one for previous. we get head 
	// pointer to indicate start and count to count number
	// of values stored in queue.
	struct Cell{
		string value;
		Cell* next;
		Cell* previous;
